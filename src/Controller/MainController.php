...
class MainController extends AbstractController
{
    ...
    /**
     * @Route("/castigatori", name="castigatori")
     */
    ...
    /**
     * @Route("/oreo-walk", name="oreo-walk")
     */
    public function oreoWalk(): Response
    {
        return $this->render('main/oreo-walk.html.twig');
    }
    ...
    public function registerCampaign(Request $request, EntityManagerInterface $em, ReceiptRegistrationRepository  $receiptRegistrationRepository): Response
    {
        ...
        if($currentDate < $campaignStartDate) {
            ...
            $jsonResponse['message'] = 'Campania "Oreo of the Day" nu a început încă. Mai ai un picuț de răbdare. Ca să te înscrii în campanie, trebuie să cumperi produse participante în valoare de minimum 15 lei, pe același bon fiscal, în perioada 12.07.2023 – 12.08.2023, și să înscrii bonul pe www.oreopromo.ro.';
            return new JsonResponse($jsonResponse, 200);
        } else if($currentDate >= $campaignEndDate) {
            $jsonResponse['messageStatus'] = 'postcampanie';
            $jsonResponse['message'] = 'Uf, ai întârziat puțin. Campania "Oreo of the Day" s-a încheiat pe data de 12.08.2023. Îți mulțumim pentru participare și te așteaptăm la următoarele campanii promoționale.';
            return new JsonResponse($jsonResponse, 200);
        }
        ...
        if($receiptWeekCount >= 10) {
            ...
            if($currentDate >= $lastWeekStart && $currentDate <= $endDate) {
                ...
                $jsonResponse['message'] = 'Campania "Oreo of the Day” se încheie săptămâna aceasta și se pare că ai atins limita de înscrieri. Mulțumim pentru participare. Verifică rezultatul tragerii la sorți și vezi dacă te afli printre câștigători!';
                return new JsonResponse($jsonResponse, 200);
            }
        ...
        if ($minute < 15) {
            ...
            $jsonResponse['prize'] = 1;
            $jsonResponse['messageStatus'] = 'prize';
            $jsonResponse['message'] = 'Felicitări! Ai câștigat un voucher Eventim în valoare de 300 LEI. Păstrează bonul fiscal, te vom contacta în curând. Înscrie și alte bonuri fiscale, pentru o șansă la premiul cel mare. Ai dreptul la cel mult zece înscrieri în fiecare săptămână de campanie.';
            return new JsonResponse($jsonResponse, 200);
        }
